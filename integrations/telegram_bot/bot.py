...
try:
    # Импорт системы управления эпиками
    from llmstruct.workspace import WorkspaceStateManager
    EPIC_INTEGRATION = True
except ImportError:
    EPIC_INTEGRATION = False
    print("⚠️ Epic integration not available")
...
class LLMStructTelegramBot:
    def __init__(self, bot_token: str, api_base: str = "http://localhost:8000"):
        ...
        # Инициализация системы эпиков
        if EPIC_INTEGRATION:
            try:
                self.workspace = WorkspaceStateManager()
                self.logger.info("📊 Epic integration enabled")
            except Exception as e:
                self.workspace = None
                self.logger.warning(f"⚠️ Epic integration failed: {e}")
        else:
            self.workspace = None
    ...
    async def handle_message(self, update):
        ...
        if chat_type in ["group", "supergroup"]:
            # В группах реагируем только на команды или упоминания
            entities = message.get("entities", [])
            for entity in entities:
                if entity.get("type") == "mention":
                    mention_text = text[entity["offset"]:entity["offset"] + entity["length"]]
                    if "@llmstruct" in mention_text.lower():  # Замените на имя вашего бота
                        bot_mentioned = True
                        # Убираем упоминание из текста
                        text = text.replace(mention_text, "").strip()
            
            # В группах обрабатываем только команды или упоминания
            if not (text.startswith("/") or bot_mentioned):
                return
        
        self.logger.info(f"📩 Parsed - chat_id: {chat_id}, text: '{text}', user: {user_name}, chat_type: {chat_type}")
...
• `/epics` - Show epic roadmap status
...
• Interests: {', '.join(profile.get('interests', [])) or 'None detected yet'}"""
        ...
        elif text.startswith("/epics"):
            self.logger.info("📊 Processing /epics command")
            epic_status = await self.get_epic_status()
            await self.send_message(chat_id, epic_status)
            
        elif text.startswith("/workspace"):
            self.logger.info("🏗️ Processing /workspace command")
            if self.workspace:
                try:
                    current_mode = self.workspace.get_current_mode()
                    context_info = self.workspace.get_context_info()
                    
                    workspace_info = f"""🏗️ **Workspace Status**

**Current Mode:** {current_mode}
**Context Level:** {context_info.get('level', 'Unknown')}
**Token Budget:** {context_info.get('token_budget', 'Unknown')}
**Active Session:** {context_info.get('session_id', 'None')}"""
                    
                    await self.send_message(chat_id, workspace_info)
                except Exception as e:
                    await self.send_message(chat_id, f"❌ Workspace error: {str(e)}")
            else:
                await self.send_message(chat_id, "❌ Workspace integration not available")
    ...
    async def get_epic_status(self) -> str:
        """Получение статуса эпиков и задач"""
        if not self.workspace:
            return "❌ Epic integration not available"
        
        try:
            # Выполняем команду для получения статуса эпиков
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                return f"❌ Epic command failed: {result.stderr}"
                
        except Exception as e:
            return f"❌ Error getting epic status: {str(e)}"
