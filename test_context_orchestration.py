...
class LLMContextTester:
    ...
    async def test_relationship_context_in_focused(self):
        """Проверяет, что в FOCUSED режиме присутствует relationship_context и он содержит модули и зависимости."""
        context = await self.load_context_for_mode("FOCUSED", 2000)
        assert "relationship_context" in context, "relationship_context отсутствует в FOCUSED режиме"
        rel = context["relationship_context"]
        assert "modules" in rel and isinstance(rel["modules"], list), "Нет modules в relationship_context"
        assert len(rel["modules"]) > 0, "relationship_context пуст"
        # Проверка, что есть зависимости и публичные функции/классы
        for m in rel["modules"]:
            assert "module_id" in m and "dependencies" in m, "Некорректная структура relationship_context"
            assert isinstance(m["dependencies"], list), "dependencies не список"
            assert "public_functions" in m and "public_classes" in m, "Нет публичных функций/классов"
        print("FOCUSED relationship_context: OK")
...
@pytest.mark.asyncio
async def test_relationship_context_focused():
    # Используем реальный orchestrator для проверки FOCUSED режима
    project_root = os.getcwd()
    orchestrator = SmartContextOrchestrator(project_root)
    context = orchestrator.get_context_for_scenario("vscode_copilot")
    assert "relationship_context" in context, "relationship_context отсутствует в FOCUSED режиме"
    rel = context["relationship_context"]
    assert "modules" in rel and isinstance(rel["modules"], list), "Нет modules в relationship_context"
    assert len(rel["modules"]) > 0, "relationship_context пуст"
