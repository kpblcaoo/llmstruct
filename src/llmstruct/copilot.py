...
from llmstruct.self_run import filter_struct, attach_to_llm_request
...
class CopilotContextManager:
    ...
    def __init__(self, project_root: str = None, config_path: str = None):
        ...
        self.config_path = config_path or self.project_root / "data" / "copilot_init.json"
    ...
    def get_context_for_event(self, event: CopilotEvent) -> Dict[str, Any]:
        ...
        if event.event_type in ["file_create", "file_edit", "file_delete"]:
            file_triggers = triggers.get("file_operations", {}).get(f"on_{event.event_type}", {})
        ...
        elif event.event_type in ["function_creation", "class_creation", "import_changes"]:
        ...
        elif event.event_type in ["cli_command_detected", "queue_operation", "task_creation"]:
            workflow_triggers = triggers.get("workflow_triggers", {}).get(event.event_type, {})
    ...
    def suggest_completion(self, 
        ...
        struct_context = self.active_contexts.get("essential", {}).get("struct.json", {})
        ...
        if struct_context:
            ...
            if current_module:
                ...
                for func in current_module.get("functions", []):
                    suggestions.append(f"def {func['name']}({', '.join(func.get('parameters', []))})")
    ...
    def validate_change(self, file_path: str, change_type: str) -> Dict[str, Any]:
        ...
        for rule in validation_rules:
            ...
            if self._matches_scope(file_path, scope) and self._matches_trigger(change_type, trigger):
    ...
    def get_workflow_suggestions(self, command: str) -> List[str]:
        ...
        cli_queue_data = self.active_contexts.get("operational", {}).get("data/cli_queue_enhanced.json", {})
...
def trigger_copilot_event(manager: CopilotContextManager, 
                         event_type: str, 
                         file_path: str = None, 
                         **kwargs) -> Dict[str, Any]:
    """
...
def smart_suggest(manager: CopilotContextManager,
                 query: str,
                 context: str = "code") -> List[str]:
    """
