...
class JavaScriptParser:
    ...
    def compute_file_metadata(
        self, filepath: str, include_hashes: bool
    ) -> Dict[str, Any]:
        try:
    ...
    def parse_module(
        self, filepath: str, root_dir: str, include_ranges: bool, include_hashes: bool
    ) -> Dict[str, Any]:
        try:
            with open(filepath, "r", encoding="utf-8") as f:
                source = f.read()
            tree = esprima.parseModule(
                source, {"loc": True, "range": True, "comment": True}
            )
        ...
        comments = [
            {
                "type": "single" if c.type == "Line" else "block",
                "text": c.value.strip(),
        ...
        for node in tree.body:
            if (
                ...
                and node.id
                and not node.id.name.startswith("_")
            ):
                ...
                doc = next(
                    (
                        c["text"]
                        for c in comments
                        if c["line"] == node.loc.start.line - 1 and c["type"] == "block"
                    ),
                    None,
                )
                func_comments = [
                    c["text"]
                    for c in comments
                    if node.loc.start.line <= c["line"] <= node.loc.end.line
                    and c["type"] == "single"
                ]
                functions.append(
                    {
