...
{"version": "0.1.0",
    ...
    "tags": ["references",
             "linking"],
    "references": [{"source": {"file": "data/ideas.json",
                               "id": "IDEA-001",
                               "artifact_id": "e1f2a3b4-c5d6-47e8-b9f0-a1b2c3d4e5f6"},
                    "target": {"file": "data/tasks.json",
                               "id": "TSK-006",
                               "artifact_id": "f2a3b4c5-d6e7-48f9-a0b1-c2d3e4f5a6b7"},
                    "type": "dependency"},
                   {"source": {"file": "data/ideas.json",
                               "id": "IDEA-002",
                               "artifact_id": "e1f2a3b4-c5d6-47e8-b9f0-a1b2c3d4e5f6"},
                    "target": {"file": "data/tasks.json",
                               "id": "TSK-027",
...
{"version": "0.2.0",
    ...
    "tags": ["principles",
             "goals"],
    "principles": [{"id": "PRINCIPLE-001",
                    "name": "Dogfooding",
                    "description": "Use llmstruct to develop llmstruct"},
                   {"id": "PRINCIPLE-002",
                    "name": "Idempotence",
                    "description": "Ensure reproducible changes"},
                   {"id": "PRINCIPLE-003",
                    "name": "Transparency",
                    "description": "Use RFC-style docs and open collaboration"}],
    "goals": ["Create universal JSON format for codebases",
...
def get_folder_structure(root_dir: str,
                         gitignore_patterns: Optional[List[str]],
                         include_patterns: Optional[List[str]],
                         exclude_patterns: Optional[List[str]],
                         exclude_dirs: Optional[List[str]]) -> List[Dict[str,
                                                                         Any]]:
    ...
    normalized_gitignore = [
        p.rstrip('/') +
        '/*' if p.endswith('/') else p for p in gitignore_patterns]
    ...
    for dir_path, dirnames, filenames in os.walk(root_dir):
        ...
        dirnames[:] = [d for d in dirnames if d not in exclude_dirs_set and not any(Path(
            dir_path).joinpath(d).match(p) for p in normalized_gitignore + (exclude_patterns or []))]
        ...
        for fname in sorted(filenames):
            ...
            if any(
                file_path.match(p) for p in (
                    include_patterns or ['*.py']) if not any(
                    file_path.match(ep) for ep in (
                        exclude_patterns or []) +
                    normalized_gitignore)):
                structure.append({
...
def build_toc_and_modules(root_dir: str,
                          include_patterns: Optional[List[str]],
                          exclude_patterns: Optional[List[str]],
                          gitignore_patterns: Optional[List[str]],
                          include_ranges: bool,
                          include_hashes: bool,
                          exclude_dirs: Optional[List[str]]) -> tuple[List[Dict[str,
                                                                                Any]],
                                                                      List[Dict[str,
                                                                                Any]]]:
...
def generate_json(root_dir: str,
                  include_patterns: Optional[List[str]],
                  exclude_patterns: Optional[List[str]],
                  gitignore_patterns: Optional[List[str]],
                  include_ranges: bool,
                  ...
                  exclude_dirs: Optional[List[str]]) -> Dict[str,
                                                             Any]:
    """Generate JSON structure for project with artifact_id, summary, and tags."""
    root_path = Path(root_dir).resolve()
    toc, modules = build_toc_and_modules(
        root_dir, include_patterns, exclude_patterns, gitignore_patterns, include_ranges, include_hashes, exclude_dirs)
...
def validate_directory(directory: str,
    ...
    for json_file in Path(directory).rglob("*.json"):
        if any(json_file.match(p) for p in include_patterns) and not any(
                json_file.match(ep) for ep in exclude_patterns):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        logging.error(
            "Usage: python json_validator.py <path> <schema.json> [--include <pattern>] [--exclude <pattern>]")
        sys.exit(1)

    path, schema_path = sys.argv[1], sys.argv[2]
    include_patterns = [sys.argv[sys.argv.index(
        "--include") + 1]] if "--include" in sys.argv else None
    exclude_patterns = [sys.argv[sys.argv.index(
        "--exclude") + 1]] if "--exclude" in sys.argv else None

    if Path(path).is_dir():
        success = validate_directory(
            path, schema_path, include_patterns, exclude_patterns)
    else:
...
def filter_json(data: Dict[str, Any], filter_key: str, filter_value: str,
...
def select_json(json_path: str, filter_key: str, filter_value: str,
                fields: Optional[List[str]] = None) -> List[Dict[str, Any]]:
